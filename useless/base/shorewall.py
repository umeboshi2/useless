...
class BlacklistRule(Rule):
    def __init__(self, mac):
        source = 'loc:%s' % self._make_shorewall_mac(mac)
        Rule.__init__(self, action='REJECT', source=source, dest='net') 
        self.proto = 'all'
        
    def _make_shorewall_mac(self, mac):
        return '~%s' % mac.replace(':', '-')

    def _parse_shorewall_mac(self, sw_mac):
        return sw_mac[1:].replace('-', ':')
...
class Shorewall(object):
    def __init__(self, loc='loc', net='net'):
        ...
        self.blacklist_rules = BlacklistRules()
        self.other_rules = []
    ...
    def get_ruleslines(self):
        ...
        lines += self._preplines(self.forward_rules) + self._preplines(self.blacklist_rules)
        lines += self._preplines(self.other_rules)
        return lines
    ...
    def blacklist_mac(self, mac):
        self.blacklist_rules.append_mac(mac)

    def blacklist_macs(self, maclist):
        for mac in maclist:
            self.blacklist_mac(mac)
